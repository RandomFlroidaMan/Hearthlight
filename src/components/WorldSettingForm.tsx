...
export function WorldSettingForm() {
  ...
  const [genre, setGenre] = useState<"fantasy" | "star-trek">("fantasy");
    ...
    <form onSubmit={handleSubmit} className="flex max-w-2xl flex-col gap-6 text-zinc-50">
      ...
      <div className="flex gap-2 text-sm">
        <button
          ...
          className={`rounded-full px-4 py-2 ${genre === "fantasy" ? "bg-zinc-50 text-zinc-950" : "border border-zinc-700 text-zinc-400"}`}
        >
          ⚔ Fantasy
        </button>
        <button
          type="button"
          onClick={() => setGenre("star-trek")}
          className={`rounded-full px-4 py-2 ${genre === "star-trek" ? "bg-zinc-50 text-zinc-950" : "border border-zinc-700 text-zinc-400"}`}
        >
          🖖 Star Trek
        </button>
      </div>
